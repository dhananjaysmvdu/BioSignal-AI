...
ATTE now consumes RDGL outputs to scale its daily shift allowance safely.

Integration details:
- Input: `state/rdgl_policy_adjustments.json` (mode + shift_percent_range)
- Mapping: relaxed×1.2, normal×1.0, tightening×0.7, locked×0.0
- Clamp: both min/max clamped to ATTE `MAX_SHIFT_PERCENT` (=3.0%); ensure max≥min
- Safety gates (no updates): trust_locked, safety_brake_engaged, or fusion RED
- Metadata emitted by ATTE:
  - `rdgl_mode_used`
  - `rdgl_shift_range_used`
  - `rdgl_scaled_percent_range`
- Audit marker: `<!-- ATTE_RDGL_INTEGRATION: USED <UTC ISO> -->` (idempotent)

Portal:
- Adaptive Thresholds card shows Learning Influence badge and RDGL-scaled shift range.

CI:
- Tuner workflow publishes `state/rdgl_impact.json` and warns if RDGL locked tuner (0,0).

Tests:
- `tests/learning/test_tuner_rdgl_integration.py` validates relaxed/normal/tightening/locked modes, marker idempotency, and fallback behavior.
